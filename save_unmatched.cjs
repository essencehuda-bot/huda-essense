...
function getUnusedMappings() {
  ...
  let output = "";
  unused.forEach(u => {
    output += `File: ${u.filename} -> "${u.raw_text}"\n`;
  });
  
  fs.writeFileSync(path.join(WORKSPACE_DIR, 'unmatched.txt'), output, 'utf8');
  console.log(`Unmatched mappings saved to unmatched.txt`);
}
